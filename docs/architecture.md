...
- `opsclaw run` command now exposes unified multi-platform runtime behavior:
  - `run serve-webhooks` hosts native HTTP ingress endpoints for Slack/Discord/Telegram webhook payload dispatch with optional shared-secret validation, in-process rate limiting, Slack signature verification, Discord signature verification, Telegram webhook secret-token verification, and bounded live retry/backoff controls
